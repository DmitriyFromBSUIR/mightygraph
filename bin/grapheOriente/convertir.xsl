	...
	<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:ev="http://www.w3.org/2001/xml-events" version="1.1" baseProfile="full" x="0" y="0" width="800" height="500" id="graphe">
		...
		<g id="liaisons">
			<xsl:for-each select="graphe/liaison">
				...
				<xsl:variable name="sourceLongueur"><xsl:value-of select="(string-length(//sommet[@id=concat($source, '')])+3)*10" /></xsl:variable>
				...
				<xsl:variable name="destLongueur"><xsl:value-of select="(string-length(//sommet[@id=concat($destination, '')])+3)*10" /></xsl:variable>
				...
				<line>
					...
					<xsl:choose>
						...
						<!-- X2 < X1 , Y1 = Y2 -->
						<xsl:when test="($midX2 - $midX1) &lt;= 0 and $midY1 = $midY2">
							<xsl:variable name="thalesY"><xsl:value-of select="0" /></xsl:variable>
							<xsl:variable name="thalesX"><xsl:value-of select="0" /></xsl:variable>
							<xsl:attribute name="cond0">true</xsl:attribute>
							
							<xsl:attribute name="x2"><xsl:value-of select="$sourcePosH + $sourceLongueur" /></xsl:attribute>
							<xsl:attribute name="y1"><xsl:value-of select="$midY1" /></xsl:attribute>
							<xsl:attribute name="x1"><xsl:value-of select="$destPosH" /></xsl:attribute>
							<xsl:attribute name="y2"><xsl:value-of select="$midY2" /></xsl:attribute>
						</xsl:when>
	
						<!-- X2 > X1 , Y1 = Y2 -->
						<xsl:when test="($midX1 - $midX2) &lt;= 0 and $midY1 = $midY2">
							<xsl:variable name="thalesY"><xsl:value-of select="50" /></xsl:variable>
							<xsl:variable name="thalesX"><xsl:value-of select="50" /></xsl:variable>
							<xsl:attribute name="cond0">true</xsl:attribute>
							
							<xsl:attribute name="x1"><xsl:value-of select="$destPosH + $destLongueur" /></xsl:attribute>
							<xsl:attribute name="y1"><xsl:value-of select="$midY1" /></xsl:attribute>
							<xsl:attribute name="x2"><xsl:value-of select="$sourcePosH" /></xsl:attribute>
							<xsl:attribute name="y2"><xsl:value-of select="$midY2" /></xsl:attribute>
						</xsl:when>
			...
			<xsl:for-each select="graphe/sommet">
				<rect height="50">
					...
					<xsl:attribute name="width">
						<xsl:value-of select="(string-length(.)+3)*10" />
					</xsl:attribute>
				...
				<text>
					...
					<xsl:attribute name="x"><xsl:value-of select="@posh + 10" /></xsl:attribute>
					<xsl:attribute name="y"><xsl:value-of select="@posv + 30" /></xsl:attribute>
					<xsl:attribute name="id">
						<xsl:value-of select="concat('text_', @id)" />
					</xsl:attribute>
					<xsl:value-of select="." />
